...
func V0MinerHandler(ctx context.Context, stateDiff *minerdiff.StateDiff) (model.PersistableList, error) {
	var out model.PersistableList
	if stateDiff.InfoChange != nil {
		infoModel, err := V0MinerInfoHandler(ctx, stateDiff)
		if err != nil {
			return nil, err
		}
	...
	if stateDiff.SectorChanges != nil {
		sectorModel, err := V0MinerSectorHandler(ctx, stateDiff)
		if err != nil {
			return nil, err
		}
	...
	if stateDiff.PreCommitChanges != nil {
		preCommitModel, err := V0MinerPreCommitHandler(ctx, stateDiff)
		if err != nil {
			return nil, err
		}
...
func V0MinerPreCommitHandler(ctx context.Context, stateDiff *minerdiff.StateDiff) (model.Persistable, error) {
	minerPreCommits := make([]*miner0.SectorPreCommitOnChainInfo, len(stateDiff.PreCommitChanges))
...
func V0MinerSectorHandler(ctx context.Context, stateDiff *minerdiff.StateDiff) (model.Persistable, error) {
	minerSectors := make([]*miner0.SectorOnChainInfo, len(stateDiff.SectorChanges))
...
func V0MinerInfoHandler(ctx context.Context, stateDiff *minerdiff.StateDiff) (model.Persistable, error) {
