...
import (
	...
	v1 "github.com/filecoin-project/lily/pkg/transform/timescale/actors/power/v1"
...
func TransformPowerState(ctx context.Context, s store.Store, version actortypes.Version, current, executed *types.TipSet, root cid.Cid) (model.Persistable, error) {
	...
	case actortypes.Version0,
		...
		actortypes.Version8,
		actortypes.Version9:
		return v1.TransformPowerState(ctx, s, version, current, executed, root)
	}
	return nil, fmt.Errorf("unsupported version : %d", version)
}

func HandlePower(ctx context.Context, current, executed *types.TipSet, addr address.Address, change *rawdiff.ActorChange, version actortypes.Version) (model.Persistable, error) {
	switch version {
	case actortypes.Version0:
		return v1_0.ExtractPowerStateChanges(ctx, current, executed, addr, change)
	case actortypes.Version2:
		return v1_2.ExtractPowerStateChanges(ctx, current, executed, addr, change)
	case actortypes.Version3:
		return v1_3.ExtractPowerStateChanges(ctx, current, executed, addr, change)
	case actortypes.Version4:
		return v1_4.ExtractPowerStateChanges(ctx, current, executed, addr, change)
	case actortypes.Version5:
		return v1_5.ExtractPowerStateChanges(ctx, current, executed, addr, change)
	case actortypes.Version6:
		return v1_6.ExtractPowerStateChanges(ctx, current, executed, addr, change)
	case actortypes.Version7:
		return v1_7.ExtractPowerStateChanges(ctx, current, executed, addr, change)
	case actortypes.Version8:
		return v1_8.ExtractPowerStateChanges(ctx, current, executed, addr, change)
	case actortypes.Version9:
		return v1_9.ExtractPowerStateChanges(ctx, current, executed, addr, change)
