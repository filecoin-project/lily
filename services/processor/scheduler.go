...
func NewScheduler(node lapi.FullNode, publisher *Publisher) *Scheduler {
	minerActorPool := work.NewWorkerPool(MinerProcessContext{}, 64, "miner_actor_jobs", redisPool)

	minerActorPool.Job("process_miner", (*MinerProcessContext).ProcessMiner)
	...
	minerActorPool.Middleware(func(mac *MinerProcessContext, job *work.Job, next work.NextMiddlewareFunc) error {
		mac.node = node
		mac.pub = publisher
		mac.log = logging.Logger("MinerProcessContext")
		return next()
	})

	minerActorQueue := work.NewEnqueuer("miner_actor_jobs", redisPool)
	minerActorPool.Start()
	return &Scheduler{
		minerActorPool:  minerActorPool,
		minerActorQueue: minerActorQueue,
	}
}

type Scheduler struct {
	minerActorPool  *work.WorkerPool
...
func (s *Scheduler) EnqueueMinerActorJob(ai indexer.ActorInfo) (*work.Job, error) {
	...
	ptsB, err := ai.ParentTipset.MarshalJSON()
	if err != nil {
		return nil, err
	}
	return s.minerActorQueue.Enqueue("process_miner", work.Q{
		"address": ai.Address.String(),
		"ts":      string(tsB),
		"pts":     string(ptsB),
		"head":    ai.Actor.Head.String(),
	})
}

type MinerProcessContext struct {
	node lapi.FullNode
...
func (mac *MinerProcessContext) Log(job *work.Job, next work.NextMiddlewareFunc) error {
	mac.log.Infow("Starting Job", "name", job.Name, "Args", job.Args)
	return next()
}

func (mac *MinerProcessContext) ParseArgs(job *work.Job) error {
...
func (mac *MinerProcessContext) ProcessMiner(job *work.Job) error {
	...
	return mac.pub.Publish(ctx, &MinerProcessResult{
		ts:               mac.tsKey,
		pts:              mac.ptsKey,
		addr:             mac.maddr,
		actor:            mactor,
