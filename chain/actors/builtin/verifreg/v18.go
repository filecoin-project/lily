...
func (s *state18) VerifiedClientsMap() (adt.Map, error) {

	return nil, fmt.Errorf("unsupported in actors v18")

}

func (s *state18) VerifiersMap() (adt.Map, error) {
	return adt18.AsMap(s.store, s.Verifiers, builtin18.DefaultHamtBitwidth)
}

func (s *state18) VerifiersMapBitWidth() int {

	return builtin18.DefaultHamtBitwidth

}

func (s *state18) VerifiersMapHashFunction() func(input []byte) []byte {
...
func (s *state18) VerifierDataCap(addr address.Address) (bool, abi.StoragePower, error) {
	return getDataCap(s.store, actorstypes.Version18, s.VerifiersMap, addr)
}

func (s *state18) RemoveDataCapProposalID(verifier address.Address, client address.Address) (bool, uint64, error) {
	return getRemoveDataCapProposalID(s.store, actorstypes.Version18, s.removeDataCapProposalIDs, verifier, client)
}

func (s *state18) ForEachVerifier(cb func(addr address.Address, dcap abi.StoragePower) error) error {
	return forEachCap(s.store, actorstypes.Version18, s.VerifiersMap, cb)
}
...
func (s *state18) ClaimMapForProvider(providerIdAddr address.Address) (adt.Map, error) {

	innerHamtCid, err := s.getInnerHamtCid(s.store, abi.IdAddrKey(providerIdAddr), s.Claims, builtin18.DefaultHamtBitwidth)
	if err != nil {
		return nil, err
	}
	return adt18.AsMap(s.store, innerHamtCid, builtin18.DefaultHamtBitwidth)

}

func (s *state18) getInnerHamtCid(store adt.Store, key abi.Keyer, mapCid cid.Cid, bitwidth int) (cid.Cid, error) {

	actorToHamtMap, err := adt18.AsMap(store, mapCid, bitwidth)
	if err != nil {
