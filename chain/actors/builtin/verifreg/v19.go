...
func (s *state19) VerifiedClientsMap() (adt.Map, error) {

	return nil, fmt.Errorf("unsupported in actors v19")

}

func (s *state19) VerifiersMap() (adt.Map, error) {
	return adt19.AsMap(s.store, s.Verifiers, builtin19.DefaultHamtBitwidth)
}

func (s *state19) VerifiersMapBitWidth() int {

	return builtin19.DefaultHamtBitwidth

}

func (s *state19) VerifiersMapHashFunction() func(input []byte) []byte {
...
func (s *state19) VerifierDataCap(addr address.Address) (bool, abi.StoragePower, error) {
	return getDataCap(s.store, actorstypes.Version19, s.VerifiersMap, addr)
}

func (s *state19) RemoveDataCapProposalID(verifier address.Address, client address.Address) (bool, uint64, error) {
	return getRemoveDataCapProposalID(s.store, actorstypes.Version19, s.removeDataCapProposalIDs, verifier, client)
}

func (s *state19) ForEachVerifier(cb func(addr address.Address, dcap abi.StoragePower) error) error {
	return forEachCap(s.store, actorstypes.Version19, s.VerifiersMap, cb)
}
...
func (s *state19) ClaimsMap() (adt.Map, error) {
...
func (s *state19) ClaimsMapBitWidth() int {
...
// TODO this could return an error since not all versions have a claims map
func (s *state19) ClaimsMapHashFunction() func(input []byte) []byte {
...
func (s *state19) ClaimMapForProvider(providerIdAddr address.Address) (adt.Map, error) {

	innerHamtCid, err := s.getInnerHamtCid(s.store, abi.IdAddrKey(providerIdAddr), s.Claims, builtin19.DefaultHamtBitwidth)
	if err != nil {
		return nil, err
	}
	return adt19.AsMap(s.store, innerHamtCid, builtin19.DefaultHamtBitwidth)

}

func (s *state19) getInnerHamtCid(store adt.Store, key abi.Keyer, mapCid cid.Cid, bitwidth int) (cid.Cid, error) {

	actorToHamtMap, err := adt19.AsMap(store, mapCid, bitwidth)
	if err != nil {
