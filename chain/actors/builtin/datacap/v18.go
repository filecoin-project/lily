...
func (s *state18) ForEachClient(cb func(addr address.Address, dcap abi.StoragePower) error) error {
	return forEachClient(s.store, actorstypes.Version18, s.VerifiedClients, cb)
}
...
func (s *state18) VerifiedClientDataCap(addr address.Address) (bool, abi.StoragePower, error) {
	return getDataCap(s.store, actorstypes.Version18, s.VerifiedClients, addr)
}

func (s *state18) VerifiedClientsMapBitWidth() int {
	return int(s.Token.HamtBitWidth)
}

func (s *state18) VerifiedClientsMapHashFunction() func(input []byte) []byte {
