...
type V8DealProposalExtractor struct{}

func (V8DealProposalExtractor) Extract(ctx context.Context, a actorstate.ActorInfo, node actorstate.ActorStateAPI) (model.Persistable, error) {
	log.Debugw("extract", zap.String("extractor", "V8DealProposalExtractor"), zap.Inline(a))
	ctx, span := otel.Tracer("").Start(ctx, "V8DealProposalExtractor.Extract")
	defer span.End()
	if span.IsRecording() {
	...
	out := make(marketmodel.MarketDealProposalsV8, len(dealProposals))
	for idx, add := range dealProposals {
		...
		out[idx] = &marketmodel.MarketDealProposalV8{
