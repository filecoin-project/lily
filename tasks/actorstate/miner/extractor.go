...
import (
	...
	_ "github.com/filecoin-project/sentinel-visor/tasks/actorstate/miner/extractors/extractors"
)
...
func init() {
	...
	for m := range extractors.ModelTaskRegistry {
...
func (m StorageMinerExtractor) Extract(ctx context.Context, a actor.ActorInfo, node actor.ActorStateAPI) (model.Persistable, error) {
	...
	ec, err := extractors.NewMinerStateExtractionContext(ctx, a, node)
	if err != nil {
	...
	for _, m := range a.Models {
		extF, found := extractors.GetModelExtractor(m)
		if !found {
