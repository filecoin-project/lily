...
func GetActorStateChanges(ctx context.Context, store adt.Store, ts, pts *types.TipSet) (ActorStateChangeDiff, error) {
	if ts.Height() == 0 {
		return GetGenesisActors(ctx, store, ts)
	}

	oldTree, err := state.LoadStateTree(store, pts.ParentState())
	if err != nil {
		return nil, err
	}
	...
	newTree, err := state.LoadStateTree(store, ts.ParentState())
	if err != nil {
		return nil, err
	}
	...
	actors, err := state.Diff(oldTree, newTree)
	if err != nil {
		return nil, err
	}
